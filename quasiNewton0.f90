...
FUNCTION g(x)
	...
	do i=1,n/2
		g(2*i-1)=(400*x(2*i-1)*((x(2*i-1)**2)-x(2*i)))+((2*x(2*i-1))-2)
...
subroutine rl_goldestein(t,x,d)
	...
	tg=0
	rl:do i=1,nrl
		if ((theta(t,x,d) <= (m1*t*thetap(zero,x,d))+theta(zero,x,d))&
		&.and.(theta(t,x,d) >= (m2*t*thetap(zero,x,d))+theta(zero,x,d))) exit rl
